...
class ProviderDashboardController extends Controller
{
    ...
    {
        $providerId = Auth::id();
        $providerLocation = Auth::user()?->serviceProvider;
        $now = Carbon::now();
        $cacheKey = 'provider.dashboard:' . $providerId . ':' . $now->format('Y-m-d-H-i');
        $dashboardData = Cache::remember($cacheKey, now()->addSeconds(60), function () use ($providerId, $now) {
            $todayStart = $now->copy()->startOfDay();
            $weekStart = $now->copy()->subDays(7);

            $tipEarningsInRange = static function (int $providerId, Carbon $start, ?Carbon $end = null): float {
                $query = WalletTransaction::where('user_id', $providerId)
            ...
            if ($totalBookings > 0) {
                $respondedBookings = Booking::where('provider_id', $providerId)
                    ->whereIn('status', ['active', 'in_progress', 'awaiting_confirmation', 'completed', 'cancelled'])
                    ->count();
            ...
            $stats['today_earnings'] = (float) Booking::where('provider_id', $providerId)
                ...
                ->sum('total');
            $stats['today_earnings'] += $tipEarningsInRange($providerId, $todayStart);

            $quickStats['week_earnings'] = (float) Booking::where('provider_id', $providerId)
                ->where('status', 'completed')
                ->where('updated_at', '>=', $weekStart)
                ->sum('total');
            $quickStats['week_earnings'] += $tipEarningsInRange($providerId, $weekStart);
            ...
            for ($i = 5; $i >= 0; $i--) {
                ...
                $amount += $tipEarningsInRange($providerId, $monthStart, $monthEnd);
            ...
            $currentMonthEarnings = (float) Booking::where('provider_id', $providerId)
                ...
                ->sum('total');
            $currentMonthEarnings += $tipEarningsInRange($providerId, $now->copy()->startOfMonth(), $now->copy()->endOfMonth());
            ...
            $lastMonthEarnings = (float) Booking::where('provider_id', $providerId)
                ...
                ->sum('total');
            $lastMonthEarnings += $tipEarningsInRange($providerId, $lastMonthStart, $lastMonthEnd);
            ...
            $activeDays = ProviderAvailability::where('user_id', $providerId)
            ...
            // If no schedule is configured, fallback to weekday working pattern.
            if ($activeDays->isEmpty()) {
                $activeDays = collect(['monday', 'tuesday', 'wednesday', 'thursday', 'friday']);
            }

            $countWorkingDays = static function (Carbon $start, Carbon $end, $activeDays): int {
                $count = 0;
                $cursor = $start->copy();

                while ($cursor->lte($end)) {
                    if ($activeDays->contains(strtolower($cursor->format('l')))) {
                        $count++;
                    }
                    $cursor->addDay();
                }
            ...
            $elapsedWorkingDaysInMonth = max(1, $countWorkingDays($monthStart, $today, $activeDays));
            $totalWorkingDaysInMonth = max(1, $countWorkingDays($monthStart, $monthEnd, $activeDays));

            $dailyRunRate = $currentMonthEarnings / $elapsedWorkingDaysInMonth;
            $forecastMonthEarnings = round($dailyRunRate * $totalWorkingDaysInMonth, 2);
            ...
            $bookingFunnel = [
                ...
                'awaiting_confirmation' => Booking::where('provider_id', $providerId)->where('status', 'awaiting_confirmation')->count(),
            ...
            $acceptedBookings = $bookingFunnel['active'] + $bookingFunnel['in_progress'] + $bookingFunnel['awaiting_confirmation'] + $bookingFunnel['completed'];
            $bookingFunnelRates = [
                'acceptance' => $totalLifecycleBookings > 0 ? (int) round(($acceptedBookings / $totalLifecycleBookings) * 100) : null,
                'completion' => $acceptedBookings > 0 ? (int) round(($bookingFunnel['completed'] / $acceptedBookings) * 100) : null,
                'cancellation' => $totalLifecycleBookings > 0 ? (int) round(($bookingFunnel['cancelled'] / $totalLifecycleBookings) * 100) : null,
            ];

            $repeatClientRows = Booking::where('provider_id', $providerId)
                ->where('status', 'completed')
                ->select('taker_id', DB::raw('COUNT(*) as completed_jobs'), DB::raw('MAX(updated_at) as last_completed_at'), DB::raw('AVG(total) as average_order_value'))
                ->groupBy('taker_id')
                ->havingRaw('COUNT(*) >= 2')
            ...
            return compact(
                'stats', 'performance', 'quickStats', 'recentJobs', 'reviews', 'monthlyEarningsTrend',
                'currentMonthEarnings', 'lastMonthEarnings', 'earningsDeltaPercent', 'dailyRunRate',
                'forecastMonthEarnings', 'elapsedWorkingDaysInMonth', 'totalWorkingDaysInMonth',
                'forecastConfidence', 'servicePerformance', 'trustScore', 'trustLevel',
                'trustMilestones', 'bookingFunnel', 'bookingFunnelRates', 'repeatClientRadar', 'growthMissions',
                'growthTips', 'unreadSupportReplies', 'unreadBookingChats', 'pendingPayoutCount',
                'pendingPayoutAmount', 'serviceAreaCount', 'featureLabData', 'smartDemandInsights'
            );
        });

        return view('pages.provider_dashboard', array_merge($dashboardData, [
            'retentionOpportunities' => $dashboardData['featureLabData']['retentionOpportunities'],
            'pricingInsights' => $dashboardData['featureLabData']['pricingInsights'],
            'nextBestActions' => $dashboardData['featureLabData']['nextBestActions'],
            'providerLocation' => $providerLocation,
        ]));
    }

    public function latestLocation(): JsonResponse
    {
        $providerLocation = ServiceProvider::where('user_id', Auth::id())->first();

        return response()->json([
            'location' => $providerLocation ? $this->formatProviderLocation($providerLocation) : null,
        ]);
    }
    ...
    public function updateLocation(Request $request): JsonResponse
    {
        ...
        $providerLocation = ServiceProvider::updateOrCreate(
            ['user_id' => $providerId],
            [
        ...
        $activeBookings = Booking::where('provider_id', $providerId)
        ...
        return response()->json([
            ...
            'location' => $this->formatProviderLocation($providerLocation),
        ]);
    }

    private function formatProviderLocation(ServiceProvider $providerLocation): array
    {
        return [
